package com.ccgui.datascience.recommender
...
object RunRecommender {
  def main(args: Array[String]): Unit = {
   ...
   // spark.sparkContext.setCheckpointDir("hdfs://tmp/")
    ...
    val base = "D:\\360安全浏览器下载\\profiledata_06-May-2005\\"
...
/*    userArtistDF.repartition(1).write.format("csv")
     .option("header", "false")//在csv第一行有属性"true"，没有就是"false"
      .option("delimiter",",")//默认以","分割
      .save("D:\\tmp\\userArtistDf")*/
  //      userArtistDF.write.format("csv").save("D:\\tmp\\userArtistDf")
  ...
  def evaluate(rawUserArtistData: Dataset[String],
    ...
    val evaluations = for (rank  <- Seq(5,30); regParam <- Seq(1.0,0.0001);
                            alpha <- Seq(1.0,40.0))
      yield {
        val model = new ALS().
          setSeed(Random.nextLong()).

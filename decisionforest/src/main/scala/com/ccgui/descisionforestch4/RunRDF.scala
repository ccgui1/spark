package com.ccgui.descisionforestch4
...
object RunRDF {
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setMaster("local[*]").setAppName("descisicforest")
    ...
    val dataWithoutHeader = spark.read.
      ...
      option("header",false).
      csv("D:\\360安全浏览器下载\\covtype.data\\covtype.data")
    ...
    val data = dataWithoutHeader.toDF(colName:_*).
...
class RunRDF(private val spark: SparkSession){
  ...
  def simpleDecisionTree(trainData: DataFrame,

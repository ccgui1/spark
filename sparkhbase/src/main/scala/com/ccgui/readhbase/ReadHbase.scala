package com.ccgui.readhbase


import org.apache.spark.{SparkConf, SparkContext}
import scala.collection.JavaConversions._
class LogAccumulator extends
  org.apache.spark.util.AccumulatorV2[String, java.util.Set[String]]
{
  ...
  override def merge(other:
                     org.apache.spark.util.AccumulatorV2[String,
...
object ReadHbase {
  def main(args: Array[String]) {
    ...
    val sum = sc.parallelize(Array("1", "2a", "3", "4b", "5", "6",
      "7cd", "8", "9"), 2).filter(line => {
      val pattern = """^-?(\d+)"""
